...
import { PROPERTY_TYPES, LISTING_TYPES, VIETNAM_CITIES, Property, CATEGORIES, ITEM_CONDITIONS } from '@/types';
...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push(`/auth/login?callbackUrl=/properties/${id}/edit`);
    }
  }, [status, router, id]);
  ...
  useEffect(() => {
    if (property) {
      reset({
        ...
        category: property.category || 'REAL_ESTATE',
        ...
        brand: property.brand || undefined,
        model: property.model || undefined,
      });
  ...
  const isOwner = property && session?.user?.id === property.userId;
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Paper sx={{ p: 4 }}>
        ...
        <form onSubmit={handleSubmit((data) => updateMutation.mutate(data))}>
          ...
          >
            {updateMutation.isPending ? <CircularProgress size={24} /> : 'Cập nhật tin đăng'}
          </Button>
        </form>
      </Paper>
    </Container>
